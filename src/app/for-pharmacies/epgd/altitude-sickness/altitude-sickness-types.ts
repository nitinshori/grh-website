...
export interface ASTravelAssessment {
  ...
  destinationAltitude: number | null; // metres; PGD v005 inclusion: above 2,500 metres
  currentAltitude: number | null; // current altitude they're at (base)
  departureDate: string; // YYYY-MM-DD
  /** PGD v005 inclusion: requesting preventative treatment or symptomatic treatment for AMS. */
  purpose: ASPurpose;
  /** PGD v005 quantity: days ascending (from first day above 2,500 m to the highest point). Needed to calculate the prevention course. */
  daysAscending: number | null;
  /** PGD v005: 1 to 2 lead-in days before ascent. A choice between the document's two values. */
  leadInDays: 1 | 2 | null;
...
export interface ASMedicalHistory {
  ...
  mildRenalImpairment: boolean; // PGD v005 caution
...
export interface ASMedicineSelection {
  ...
  /** PGD v005: 6 treatment tablets may be supplied in addition to the prevention course where descent is difficult. */
  includeTreatmentCourse: boolean;
  /** PGD v005: quantity supplied in tablets. Max 14 prevention, 6 treatment, 20 total. */
  quantityTablets: number | null;
  /** PGD v005 records: name and brand of medication. */
  brand: string;
  /** PGD v005: patient told that use for AMS is off-label, recorded. */
