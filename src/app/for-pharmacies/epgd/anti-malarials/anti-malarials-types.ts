...
export interface AMTravelAssessment {
  ...
  /** PGD v011: destination risk assessment from current NaTHNaC / TravelHealthPro guidance is an inclusion criterion. */
  riskAssessmentCompleted: boolean;
  /** PGD v011 records: the source consulted for the destination recommendation. */
  riskAssessmentSource: string;
  /** PGD v011: body weight in kg. Weight, not age, determines dose and product strength. */
  weightKg: number | null;
  /** PGD v011 inclusion: able and willing to complete the whole course including the post-travel tail. */
...
export interface AMMedicalHistory {
  ...
  /** PGD v011: any febrile illness now, or presenting for treatment of suspected/confirmed malaria. Exclusion, refer same day. */
  currentFeverOrSuspectedMalaria: boolean;
  /** PGD v011: fever within the last 12 months after travel to a malarious area, not investigated with a blood film. Exclusion. */
...
export interface AMMedicineSelection {
  ...
  /** PGD v011 records: quantity supplied. Pre-filled from the calculated course and validated against it. */
  quantity: number | null;
  /** The calculated course length including the tail, as text, set with the quantity. */
  courseCalculation: string;
  /** PGD v011 mefloquine arm: a divided dose may only be supplied from a scored tablet. */
  scoredTabletConfirmed: boolean;
  /** PGD v011 records: batch number and expiry date. */
  batchNumber: string;
  expiryDate: string;
  reason: string;
}
...
export interface AMCounselling {
  ...
  /** PGD v011: keep taking it for the post-travel tail (7 days A/P, 4 weeks doxycycline and mefloquine). */
  completeCourseAdvised: boolean;
  /** PGD v011 mefloquine: STOP and seek advice at the first neuropsychiatric symptom, including insomnia and abnormal dreams. */
