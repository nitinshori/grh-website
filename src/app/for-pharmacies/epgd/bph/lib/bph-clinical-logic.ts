...
// Aligned to the Tamsulosin 400mcg MR capsules for BPH PGD, version 002,
...
export function getAllAlerts(state: BPHConsultationState): ClinicalAlert[] {
  ...
  if (state.patient.age !== null && state.patient.age < 45) {
    alerts.push({
      ...
      detail: "Excluded under PGD v002. Refer to the GP for diagnosis.",
    });
  }
  ...
  if (mh.orthostasisHistory) {
    alerts.push({
      ...
      detail: "Excluded under PGD v002. Tamsulosin can cause first-dose hypotension and syncope. Refer.",
    });
  }
  ...
  if (mh.plannedCataractSurgery) {
    alerts.push({
      ...
      detail: "Excluded under PGD v002 because of the risk of intraoperative floppy iris syndrome (IFIS). Refer to the GP; the surgeon must be told of any alpha-blocker use.",
    });
  }
  ...
  if (ms.supplyType === "continuation") {
    ...
    if (!ms.gpExaminedSinceStart) {
      alerts.push({
        ...
        detail: "PGD v002 maximum treatment period. Do not continue supply until the GP has examined the patient.",
      });
    }
