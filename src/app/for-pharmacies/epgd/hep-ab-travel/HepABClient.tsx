...
import {
  initialHepABPatientDetails,
  initialHepABConsent,
  initialHepABSummary,
} from './hep-ab-travel-types';
import {
  getHepABClinicalAlerts,
  shouldBlockConsultation,
  getAdministrationGuidance,
} from './hep-ab-travel-clinical-logic';
import {
  validateHepABPatientStep,
  validateHepABTravelStep,
  validateHepABConsentStep,
  validateHepABAdministrationStep,
  validateHepABSummaryStep,
} from './hep-ab-travel-validation';
...
export function HepABClient() {
  ...
  const [patientDetails, setPatientDetails] = useState<HepABPatientDetails>(
    initialHepABPatientDetails
  );

  const [consent, setConsent] = useState<HepABConsent>(initialHepABConsent);
  ...
  const [summary, setSummary] = useState<HepABSummary>(
    initialHepABSummary()
  );
  ...
  const handlePatientDetailsChange = useCallback(
    (field: keyof HepABPatientDetails, value: any) => {
  ...
  const clinicalAlerts = useMemo(() => {
    return getHepABClinicalAlerts(patientDetails, medicalHistory);
  ...
  const patientValidationError = useMemo(() => {
    return validateHepABPatientStep(patientDetails);
  }, [patientDetails]);

  const consentValidationError = useMemo(() => {
    return validateHepABConsentStep(consent);
  }, [consent]);

  const administrationValidationError = useMemo(() => {
    return validateHepABAdministrationStep(summary);
  }, [summary]);

  const summaryValidationError = useMemo(() => {
    return validateHepABSummaryStep(summary);
  }, [summary]);

  const travelValidationError = useMemo(() => {
    return validateHepABTravelStep(patientDetails, travelAssessment);
  ...
  const handleNewConsultation = useCallback(() => {
    ...
    setPatientDetails(initialHepABPatientDetails);
    setConsent(initialHepABConsent);
    setSummary(initialHepABSummary());
    ...
    <>
        ...
        >
          <div className="space-y-4">
            ...
            <SelectInput
              ...
              onChange={(v) =>
                handlePatientDetailsChange('travelReason', v as HepABPatientDetails['travelReason'])
              }
              options={[
            ...
            <Checkbox
              ...
              description="Confirm destination is appropriate for HepAB vaccination"
            />
      ...
      {currentStep === 3 && (
        ...
        >
          <div className="space-y-4">
            <Checkbox
              label="Anaphylaxis to previous HepAB dose"
            ...
            {clinicalAlerts.length === 0 && !isBlocked && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <p className="text-green-700 text-sm font-semibold">
                  No clinical alerts identified. Patient is suitable for HepAB vaccination.
                </p>
              </div>
            )}
