...
// version 010, issued 14 September 2026. "stop" = exclusion criterion (do not
...
export function getAllAlerts(state: MounjaroConsultationState): ClinicalAlert[] {
  ...
  if (fivePercentRuleApplies(state)) {
    alerts.push({
      ...
      detail:
        "PGD v010 monitoring row: a decision is required on whether to continue treatment, taking into account the benefit-risk profile in this patient. Record the decision and the reasoning before any further supply.",
    });
  }
...
export function calculateDoseRecommendation(state: MounjaroConsultationState): DoseRecommendation | null {
  // Mounjaro titration schedule (PGD v010): 2.5 mg for 4 weeks, then 5 mg; further
