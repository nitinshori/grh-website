...
export default function EPGDIndexPage() {
  const categories = categoryOrder.filter((cat) =>
    epgds.some((e) => e.category === cat)
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2 max-w-2xl">
            Digital clinical decision support tools for pharmacists. Each ePGD
        ...
        <div className="space-y-10">
          {categories.map((cat) => {
            const items = epgds.filter((e) => e.category === cat);
            return (
              <div key={cat}>
                <h2 className="text-lg font-bold text-navy-900 mb-4 flex items-center gap-2">
                  <span
                    className={`w-3 h-3 rounded-full ${items[0].color}`}
                  />
                  {cat}
        ...
        <div className="mt-10 text-center">
          <p className="text-xs text-gray-400 max-w-lg mx-auto">
            Each ePGD follows NICE guidelines and is designed as a clinical
            decision support aid — the pharmacist retains full clinical
            responsibility.
          </p>
        </div>
      </div>
