...
import {
  initialYellowFeverPatientDetails,
  initialYellowFeverConsent,
  initialYellowFeverSummary,
} from './yellow-fever-types';
import {
  getYellowFeverClinicalAlerts,
  shouldBlockConsultation,
  getAdministrationGuidance,
} from './yellow-fever-clinical-logic';
import {
  validateYellowFeverPatientStep,
  validateYellowFeverStep,
  validateYellowFeverConsentStep,
  validateYellowFeverAdministrationStep,
  validateYellowFeverSummaryStep,
} from './yellow-fever-validation';
...
export function YellowFeverClient() {
  ...
  const [patientDetails, setPatientDetails] = useState<YellowFeverPatientDetails>(
    initialYellowFeverPatientDetails
  );

  const [consent, setConsent] = useState<YellowFeverConsent>(initialYellowFeverConsent);
  ...
  const [summary, setSummary] = useState<YellowFeverSummary>(
    initialYellowFeverSummary()
  );
  ...
  const handlePatientDetailsChange = useCallback(
    (field: keyof YellowFeverPatientDetails, value: any) => {
  ...
  const clinicalAlerts = useMemo(() => {
    return getYellowFeverClinicalAlerts(patientDetails, medicalHistory);
  ...
  const patientValidationError = useMemo(() => {
    return validateYellowFeverPatientStep(patientDetails);
  }, [patientDetails]);

  const consentValidationError = useMemo(() => {
    return validateYellowFeverConsentStep(consent);
  }, [consent]);

  const administrationValidationError = useMemo(() => {
    return validateYellowFeverAdministrationStep(summary);
  }, [summary]);

  const summaryValidationError = useMemo(() => {
    return validateYellowFeverSummaryStep(summary);
  }, [summary]);

  const travelValidationError = useMemo(() => {
    return validateYellowFeverStep(patientDetails, travelAssessment);
  ...
  const handleNewConsultation = useCallback(() => {
    ...
    setPatientDetails(initialYellowFeverPatientDetails);
    setConsent(initialYellowFeverConsent);
    setSummary(initialYellowFeverSummary());
    ...
    <>
        ...
        >
          <div className="space-y-4">
            ...
            <SelectInput
              ...
              onChange={(v) =>
                handlePatientDetailsChange('travelReason', v as YellowFeverPatientDetails['travelReason'])
              }
              options={[
            ...
            <Checkbox
              ...
              description="Confirm destination is appropriate for YellowFever vaccination"
            />
      ...
      {currentStep === 3 && (
        ...
        >
          <div className="space-y-4">
            <Checkbox
              label="Anaphylaxis to previous YellowFever dose"
            ...
            {clinicalAlerts.length === 0 && !isBlocked && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <p className="text-green-700 text-sm font-semibold">
                  No clinical alerts identified. Patient is suitable for YellowFever vaccination.
                </p>
              </div>
            )}
...
export default YellowFeverClient;
