...
export function getYellowFeverClinicalAlerts(
  patient: YellowFeverPatientDetails,
  medicalHistory: {
  ...
  if (medicalHistory.anaphylaxisToVaccine) {
    alerts.push({
      ...
      message: 'Anaphylaxis to previous YellowFever dose',
      detail:
  ...
  if (!patient.departureDate) {
    alerts.push({
      ...
      detail:
        'YellowFever must be given at least 10 days before travel to high-risk areas and within 3 years for Saudi entry. Confirm timing.',
    });
  } else {
    ...
    if (daysUntilTravel < 10 && daysUntilTravel >= 0) {
      alerts.push({
        ...
        detail: `Only ${daysUntilTravel} days until travel. YellowFever should ideally be given ≥10 days before departure. Discuss risk/benefit with patient.`,
      });
    }
  ...
  if (patient.previousYellowFeverDose && patient.previousDoseDate) {
    ...
    if (yearsElapsed >= 5) {
      alerts.push({
        ...
        message: '5 years or more since previous YellowFever dose',
        detail:
...
export function getYellowFeverDoseRecommendation(
  patient: YellowFeverPatientDetails
): string {
